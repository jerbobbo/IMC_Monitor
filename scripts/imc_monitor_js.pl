...
$monitorlogfile ="IMC_Monitor.log";
$monitorcsvfile ="Monitor.sansay.csv";

#assign file handles

open( CDRCLEAN, ">$monitorcsvfile");
open( OUTFILELOG, ">>$monitorlogfile");
...
@files = <../seed/cdr/*>;
foreach $file (@files) {
	eval { open( INFILE, "<$file") ; } ;
		if ($@)
		{
		print OUTFILELOG "Can't open $file  : $timestamp \n";
		next;
		}
		flock( INFILE, 2);

($SetupTime, $ConnectTime, $DisconnectTime, $CallingID, $CalledID, $Member, $CountryCode, $RegionID, $ConfID, $CallOrigin, $CallType, $GWip, $RxdCalledNumb, $RxdCallingNumb, $FinalCalledNumb, $FinalCallingNumb, $RemoteAddress, $RemoteMedAddress, $Username, $NAStrunk, $NASd, $SessionTime, $DisconnectCause, $OutPackets, $InPackets, $OutOctets, $InOctets, $AcctStatusType,$regionidprim,$v_t) = ("","","","","","","","","","","","","","","","","","","","","","","","","","","","","","","","");

	READLINE: while($line = <INFILE>){
		@cdr_values = split(";", $line);
		next READLINE if (@cdr_values != 71);#to avoid processing the last line of the file whcih is an empty line
		@setupdate_val = split(" ", $cdr_values[6]);
		$SetupTime = "$setupdate_val[4]-$monthNum{$setupdate_val[1]}-$setupdate_val[2] $setupdate_val[3]";
		next READLINE if ($setupdate_val[1] eq '');

		if (($cdr_values[7] eq 'NA') || ($cdr_values[7] eq '')) {
			$ConnectTime = $SetupTime;
		} else {
			@connectdate_val = split(" ", $cdr_values[7]);
			$ConnectTime = "$connectdate_val[4]-$monthNum{$connectdate_val[1]}-$connectdate_val[2] $connectdate_val[3]";
		}
		if (($cdr_values[8] eq 'NA') || ($cdr_values[8] eq '')) {
			$DisconnectTime = $SetupTime;
		} else {
			@disconnectdate_val = split(" " ,$cdr_values[8]);
			$DisconnectTime = "$disconnectdate_val[4]-$monthNum{$disconnectdate_val[1]}-$disconnectdate_val[2] $disconnectdate_val[3]";
		}

		#next READLINE if ($SetupDate =~ /2000/);
		$GWip =$cdr_values[18];
		$GWip =~ s/:1720//;
		...
		#next READLINE if ($RemoteAddress eq '');
...
# 	READLINE: while($line = <INFILE>){
# 		@cdr_values=split(";",$line);
# 		next READLINE if (@cdr_values<3);#to avoid processing the last line of the file whcih is an empty line
# 		@setupdate_val=split(" " ,$cdr_values[6]);
