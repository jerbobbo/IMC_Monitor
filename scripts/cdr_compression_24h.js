...
var insertQuery = `
insert into accounting_summary_24h select
batch_time_24h,
...
where batch_num > (select max(batch_num) from accounting_summary_24h)
group by batch_time_24h,
origin_member_id,
term_member_id,
origin_address_id,
term_address_id,
...
var deleteSummary24hQuery = `
  delete from accounting_summary_24h
  where TIMESTAMPDIFF(DAY, (select max(batch_time_24h)
  from accounting_summary), batch_time_24h) < -740;
  `;
...
.then( () => conn.query(deleteSummary24hQuery) )
.then( () => {
  console.log('cdr_commpression_24h ran successfully');
  return pool.end();
})
.catch(console.log);
